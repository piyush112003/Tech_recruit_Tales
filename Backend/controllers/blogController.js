...
export const blogPost = catchAsyncErrors(async (req, res, next) => {
  ...
  const [mainImageRes, paraOneImageRes, paraTwoImageRes, paraThreeImageRes] =  // sb promises ekasth chla diye 
    await Promise.all(uploadPromises);  // conresponding error will go to corresponding array destruption e.g eoor in paraOnImaesRes upload will go to paraOneImageRes
  ...
  // dont know if user provides paras hence check
  if (paraOneImageRes) {
