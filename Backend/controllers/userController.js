...
export const register = catchAsyncErrors(async (req, res, next) => {
  ...
  let user = await User.findOne({ email });   // here I used const it showed error coz we redefine it hence resolved by let
  if (user) {
  ...
  user = await User.create({    // redefine user again
    name,
    email,
    password,
    phone,
...
export const logout = catchAsyncErrors((req, res, next) => {
  res.status(200)
   .cookie("token", "", {   // cookie ka naam token hai coz jwttoken me dekho udhr bhbi token likha hai
...
export const getMyProfile = catchAsyncErrors((req, res, next) => {
  const user = req.user;   // idhr req.user use kr skte hia czo vo isAuthentivated mai hai jo ki middleware hai getMyprofile ak its more like child class and parent class 
  res.status(200).json({
    success: true,
    user,
  });
